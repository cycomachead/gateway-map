...
import type { Building, Floor, Point, Poi, Space, SpaceCategory } from './types';

/**
 * Gateway building (UC Berkeley College of Computing, Data Science, and Society).
 *
 * Level 4 is traced from the wayfinding signs photographed on the floor:
 *  - the whole-floor sign ("Level Four") gives the outline, atrium stair and cores;
 ...
 *
 * Floor coordinates are pixels of the whole-floor sign photo (2000 × 1057).
 */
...
// ---------------------------------------------------------------------------
// Outline corners (floor frame), clockwise on screen
// ---------------------------------------------------------------------------
const P = {
  swNW: [40, 352] as XY, // Southwest wing, north-west corner
  swNE: [415, 243] as XY, // Southwest wing, north-east corner (by the NW stair)
  notchS: [455, 410] as XY, // where the wall starts curving into the facade notch
  notchE: [682, 440] as XY, // end of the notch curve, base of the NE wing's west wall
  neNW: [683, 168] as XY, // Northeast wing, north-west corner
  neN: [1325, 22] as XY, // Northeast wing, north corner
  tip: [1990, 718] as XY, // east tip
  tipSW: [1582, 777] as XY, // tip, south-west corner
  sBend: [1520, 600] as XY, // start of the S-curve along the atrium corridor
  sEnd: [1145, 640] as XY, // S-curve turns south towards the Southwest wing
  eMid: [1163, 750] as XY, // Southwest wing's east wall begins
  swSE: [1180, 850] as XY, // Southwest wing, south-east corner
  swSW: [260, 975] as XY, // Southwest wing, south-west corner
};

/** Named straight wall segments; each is a piece of the outline below. */
const walls = {
  swNorth: wall(P.swNW, P.swNE),
  swEast: wall(P.swNE, P.notchS),
  neWest: wall(P.notchE, P.neNW),
  neNorth: wall(P.neNW, P.neN),
  neDiagonal: wall(P.neN, P.tip),
  neTip: wall(P.tip, P.tipSW),
  neInner: wall(P.tipSW, P.sBend),
  swEastLower: wall(P.eMid, P.swSE),
  swSouth: wall(P.swSE, P.swSW),
  swWest: wall(P.swSW, P.swNW),
...
const outline4: Point[] = [
  xy(P.swNW),
  xy(P.swNE),
  xy(P.notchS, 0.87), // concave facade notch, bowing towards the atrium
  xy(P.notchE),
  xy(P.neNW),
  xy(P.neN),
  xy(P.tip),
  xy(P.tipSW),
  xy(P.sBend, 0.716), // S-curve along the atrium corridor
  xy(P.sEnd, 0.25),
  xy(P.eMid),
  xy(P.swSE),
  xy(P.swSW),
...
const floors: Floor[] = [{ id: '4', label: '4', level: 4, width: W, height: H, outline: outline4 }];
...
// Room builders
// ---------------------------------------------------------------------------
type Wing = 'Northeast' | 'Southwest';

type RoomSpec = {
  cat: SpaceCategory;
  name?: string;
  /** Map label; numbered rooms default to their number. */
  label?: string;
  description?: string;
  tags?: string[];
};
...
/** Adds a room whose polygon is already in floor coordinates. */
function place(wing: Wing, id: string, polygon: Point[], spec: RoomSpec) {
  const number = /^\d+$/.test(id) ? id : undefined;
  spaces.push({
    id: `4-${id.toLowerCase().replace(/\s+/g, '-')}`,
    name: spec.name ?? `${kindName[spec.cat] ?? 'Room'} ${id}`,
    label: spec.label ?? number ?? spec.name ?? id,
    floorId: '4',
    category: spec.cat,
    wing,
    polygon,
    description: spec.description,
    tags: [wing.toLowerCase(), ...(number ? [number] : []), ...(spec.tags ?? [])],
  });
}

/** Interior room traced in detail-sign pixels. */
const traced = (wing: Wing, m: Affine, id: string, poly: Point[], spec: RoomSpec) =>
  place(wing, id, transform(m, poly), spec);

/** Perimeter rooms along wall[t0..t1]. */
...
// ---------------------------------------------------------------------------
{
  ...
  const { swNorth, swEast, swSouth, swWest, swEastLower } = walls;

  // North wall: 4431 (corner, meeting) … 4447 (corner, meeting)
  place(w, '4431', cornerRoom(swWest, 0.92, swNorth, 0.14), { cat: 'meeting' });
  ...
  // West wall (top → bottom). Wall runs south→north, so parameters count from the SW corner.
  place(w, '4420', offWall(swWest, 0.81, 0.92, 98), { cat: 'open' });
  perimeterRow(w, swWest, 0.497, 0.788, 72, ['4411', '4413', '4415', '4417', '4419']);
  place(w, '4389', offWall(swWest, 0.414, 0.483, 65), { cat: 'meeting' });
  perimeterRow(w, swWest, 0.229, 0.4, 62, ['4383', '4385', '4387']);
  place(w, '4380', offWall(swWest, 0.137, 0.224, 85), { cat: 'open' });
  ...
  place(w, '4375', cornerRoom(swSouth, 0.925, swWest, 0.109), { cat: 'meeting' });
...
spaces.push({
  ...
  polygon: ellipse(900, 520, 150, 90),
...
const pois: Poi[] = [
  poi('ne-entrance', 'Northeast entrance', 'exit', { x: 1135, y: 325 }),
  poi('sw-entrance', 'Southwest entrance', 'exit', { x: 632, y: 690 }),
  poi(
    'ne-porch',
  ...
  poi('nw-stair', 'Northwest stair', 'stairs', { x: 428, y: 380 }),
  poi('east-stair', 'East stair', 'stairs', { x: 1548, y: 675 }),
];
